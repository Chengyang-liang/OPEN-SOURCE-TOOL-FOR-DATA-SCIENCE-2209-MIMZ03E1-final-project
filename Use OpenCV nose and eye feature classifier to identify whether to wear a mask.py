...
def nose_dection(img):
    ...
    nose_cascade.load("D:\opencv\haarcascade_mcs_nose.xml")
...
def eye_dection(img):
    ...
    eyes_cascade.load("D:\opencv\haarcascade_eye_tree_eyeglasses.xml")
...
def main():
    image = cv2.imread("backg.jpg")
    cv2.imshow('skin', image)
    cv2.createTrackbar("Hmin", "skin", 0, 90, empty)
    cv2.createTrackbar("Hmax", "skin", 25, 90, empty)
    capture = cv2.VideoCapture(0)
    while True:
        ref, img = capture.read()  # 打开摄像头
        img_hsv = img
        image_nose, flag_nose = nose_dection(img)  # 进行口罩检测
        if flag_nose == 1:  # 当检测到鼻子的时候，判断未戴口罩
            frame = cv2.putText(image_nose, "NO MASK", (10, 30), cv2.FONT_HERSHEY_COMPLEX, 0.9, (0, 0, 255),
                                1)  # 在图片上标记
        ...
        if flag_nose == 0:  # 未检测鼻子，进行眼睛检测
            ...
            H, S, V = cv2.split(hsv)
            Hmin = cv2.getTrackbarPos("Hmin", 'skin')
            ...
            if len(eyes) > 1:  # 判断是否检测到两个眼睛，其中eyes[0]为左眼坐标
                ...
                mask_x_end = max(eyes[0][0], eyes[1][0]) + \
                             eyes[list([eyes[0][0], eyes[1][0]]).index(max(list([eyes[0][0], eyes[1][0]])))][
                                 2]  # 把右眼x坐标 + 右眼宽度作为口罩区域x的终止坐标
                ...
                mask_y_end = max(eyes[0][1] + 2 * eyes[0][3], eyes[1][1] + 2 * eyes[1][3]) + 20
                if mask_y_end > img_eye.shape[1]:
                    mask_y_end = img_eye.shape[1]
                frame = cv2.rectangle(img_eye, (mask_x_begin, mask_y_begin), (mask_x_end, mask_y_end), (255, 0, 0),
                                      2)
                total_mask_pixel = 0
                total_face_pixel = 0
                ...
                if total_mask_pixel > total_face_pixel:
                    frame = cv2.putText(img_eye, "HAVE MASK", (mask_x_begin, mask_y_begin - 10),
                                        cv2.FONT_HERSHEY_COMPLEX, 0.9, (0, 0, 255), 1)  # 绘制
                if total_mask_pixel < total_face_pixel:
                    frame = cv2.putText(img_eye, "NO MASK", (mask_x_begin, mask_y_begin - 10), cv2.FONT_HERSHEY_COMPLEX,
                                        0.9, (0, 0, 255), 1)
